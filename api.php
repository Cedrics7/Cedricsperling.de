...
switch ($action) {
    ...
    // ... alle anderen cases bleiben unverändert
...
function handlePlaceOrder($conn) {
    ...
    // E-Mail validieren, falls als Gast bestellt wird
    ...
    // Bei eingeloggtem User die Daten aus der DB holen
    if (isUserLoggedIn()) {
    ...
    try {
        // Bestellung in 'orders' Tabelle eintragen
        ...
        // Bestell-Items in 'order_items' Tabelle eintragen
...
function sendOrderConfirmationEmail($email, $name, $order_id, $items, $total) {
    ...
    try {
        ...
        $body .= "<table border='1' cellpadding='5' cellspacing='0'><tr><th>Produkt</th><th>Menge</th><th>Preis</th></tr>";
        foreach ($items as $item) {
            $body .= "<tr><td>" . $item['name'] . "</td><td>" . $item['quantity'] . "</td><td>" . number_format($item['price'] * $item['quantity'], 2, ',', '.') . " €</td></tr>";
        }
        $body .= "</table>";
        $body .= "<p><b>Gesamtsumme: " . number_format($total, 2, ',', '.') . " €</b></p>";

        $mail->isHTML(true);
        $mail->Body = $body;
        $mail->AltBody = 'Vielen Dank für Ihre Bestellung! Ihre Bestellnummer ist #' . $order_id . '. Gesamtsumme: ' . $total . ' €';

        $mail->send();
    } catch (Exception $e) {
        // Fehler stillschweigend protokollieren, da die Bestellung an sich erfolgreich war
        error_log("Confirmation mail for order #$order_id to $email failed: " . $mail->ErrorInfo);
    }
}

// --- Restliche Funktionen (handleLogin, handleRegister, etc. bleiben wie in der letzten Version) ---
// Hier sind sie der Vollständigkeit halber nochmal eingefügt
...
function handleLogin($conn) {
    ...
    if ($user = $result->fetch_assoc()) {
        if (password_verify($password, $user['password_hash'])) {
            ...
            // Dieser Aufruf funktioniert jetzt, da die Tabelle existiert
